...
namespace MonadLib
{
    public static class MonadAdapterRegistry
    {
        ...
        static MonadAdapterRegistry()
        {
            ...
            var monadAdapterClosedTypeString = typeof(WriterMonadAdapter<ListMonoid<string>, string>);
            var monadAdapterOpenType = monadAdapterClosedTypeString.GetGenericTypeDefinition();
            var listMonoidOpenType = typeof (ListMonoid<>);

            var myType = typeof (int);
            var listMonoidClosedTypeMyType = listMonoidOpenType.MakeGenericType(myType);
            var monadAdapterClosedTypeMyType = monadAdapterOpenType.MakeGenericType(listMonoidClosedTypeMyType, myType);
            var monadAdapter = (WriterMonadAdapter<ListMonoid<int>, int>)Activator.CreateInstance(monadAdapterClosedTypeMyType);
            var monad = monadAdapter.Return(42);

            //RegisterWithSubtype(typeof(Writer<ListMonoid<string>, string, Unit>), typeof(ListMonoid<>), typeof(WriterMonadAdapter<ListMonoid<string>, string>));
        }
